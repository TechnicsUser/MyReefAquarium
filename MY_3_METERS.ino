...
void loop() {
  if (millis() - runTime >= 0L) { // Execute every 2s
    ...
    int xpos = 0, ypos = 5, gap = 4, radius = 75;
    ...
    xpos = gap + ringMeter(maxPh,minPh,minTemp, maxTemp,reading, 0, 500, xpos, ypos, radius, "mA", GREEN2RED); // Draw analogue meter

    reading = 20 + 30 * sineWave(d+60);
    xpos = gap + ringMeter(maxPh,minPh,minTemp, maxTemp,reading, -10, 50, xpos, ypos, radius, "degC", MYCOL); // Draw analogue meter

    reading = 50 + 50 * sineWave(d + 120);
   ringMeter(maxPh,minPh,minTemp, maxTemp,reading, 0, 100, xpos, ypos, radius, "%RH", BLUE2BLUE); // Draw analogue meter
...
// #########################################################################
int ringMeter(float maxPh,  float minPh,float minTemp,float maxTemp ,int value, int vmin, int vmax, int x, int y, int r, char *units, byte scheme)
{
    ...
    if (i < v) { // Fill in coloured segments with 2 triangles
      switch (scheme) {
        ...
        case 5: colour = rainbow(map(i, -angle, angle, 127, 63)); break; // Red to green (low battery etc)
               case 6: colour = rainbow(map(i, -angle, angle, 127, 0)); break; // Full spectrum blue to red
  ...
  if (r > 84) tft.drawString(units, x, y + 60, 4); // Units display
     ...
     tft.drawFloat(maxPh,1,130, 245, 6); // Value in middle 
  tft.drawFloat(minPh,1, 130, 245, 6); // Value in middle 

  tft.drawFloat(value,3,x, y - 45, 6); // Value in middle 
  
       tft.drawFloat(maxTemp,1, 130,265 , 6); // Value in middle 
     tft.drawFloat(minTemp,1,190, 265 , 6); // Value in middle 
